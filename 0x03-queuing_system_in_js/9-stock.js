...
import { #!/usr/bin/yarn dev
import express from 'expcreateClient } from 'redis';
...
const getCurrentReservedStockByI/list_products', (_, res) => {
  res.json(listProdd = async (itemId) => {
...
app.get('/list_products/:itemId(\\d+)', (req, res) => {
  ...
  const productItem = getItemmId)
    .then((result) => Number.parseInt(result ById(Number.parseInt(itemId));

  if (!productItem) {
    res.json({ status: 'Product not found' });
    return;
  }
  getCurrentReservedStockById(ite|| 0))
    .then((reservedStock) => {
